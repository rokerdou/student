...
function OnFronted()
	local stu = sandbox:get("student")
	if stu~=nil then
		stuid:setText(stu:getId());
		stuname:setText(stu:getName());
		stupwd:setText(stu:getPassword())
		if stu:getSex()=="男" then
			sex1:setIscheck(true)
			sex2:setIscheck(false)
		else
			sex2:setIscheck(true)
			sex1:setIscheck(false)
		end
		stuclassid:setText(stu:getClassid())
		stutime:setText(stu:getClassgrade())
		stucardid:setText(stu:getCardid())
		stuprovince:setText(stu:getProvince())
